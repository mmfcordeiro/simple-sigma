    ...
    function forward(increment, once) {        
        if (Session.get('play') == 1) {
            ...
            Meteor.call('render', new Date(+newbegin), new Date(+newend), Session.get("token"), function(err, response) {
                ...
                Meteor.call('communities', Session.get("token"), function(err, response) {
                });
    ...
    Template.container.rendered = function() {
        if (!this._rendered) {
            ...
            var updateEdge = function(document) {
                ...
                var source_node = getNode(document.source);
                var target_node = getNode(document.target);

                if( colors[document.community.source] === undefined ) {
                    colors[document.community.source] = randomColor();
                } 
                else {
                    source_node.color = colors[document.community.source];
                }
                
                if( colors[document.community.target] === undefined ) {
                    colors[document.community.target] = randomColor();
                }
                else {
                    target_node.color = colors[document.community.target];
                }
        ...
        Meteor.methods({
            render: function(begin, end, token) {
                ...
                EdgesCopy.find({
                    ...
                    edge.community = {};
                    edge.community.source = 0;
                    edge.community.target = 0;
            ...
            communities: function(token) {

                // calculate communities
                console.time("calculate communities"); 
                var node_data = [];
                var edge_data = [];
                Edges.find({'token': token}).forEach(function (document) {
                    if (node_data.indexOf(document.source) === -1) {
                        node_data.push(document.source);
                    }

                    if (node_data.indexOf(document.target) === -1) {
                        node_data.push(document.target);
                    }
                ...
                // Edges.find({'token': token}).forEach(function (document) {
                //     Edges.update({"_id" :document._id },{$set : {"community" : { 'source': result[document.source], 'target': result[document.target] }}});
                // });

                console.timeEnd("calculate communities");
